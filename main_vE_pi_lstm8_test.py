...
def select_tracks_for_prediction(boxes, track_ids, class_ids, confs, risk_results=None):
    """ROI/pair/risk ????? LSTM ?? ?? track? ?? MAX_LSTM_TRACKS? ??."""
    items = list(zip(boxes, track_ids, class_ids, confs))
    if not items:
        return []

    item_by_tid = {t: (b, t, c, cf) for b, t, c, cf in items}
    persons = [(b, t, c, cf) for b, t, c, cf in items if c == 0]
    vehicles = [(b, t, c, cf) for b, t, c, cf in items if c in VEHICLE_CLASSES]
    roi_persons = [it for it in persons if is_point_in_roi_fast(float(it[0][0]), float(it[0][1]))]
    ref_persons = roi_persons if roi_persons else persons

    selected = {}

    def add_track(tid):
        if tid in item_by_tid and tid not in selected:
            selected[tid] = item_by_tid[tid]
        return len(selected) >= MAX_LSTM_TRACKS

    # 1??: ?? sample?? danger/caution?? ?? pair? track
    if risk_results:
        ranked_risks = sorted(risk_results, key=lambda r: (not r["danger"], r.get("ttc_sec", 999.0)))
        for r in ranked_risks:
            if add_track(r["ped_id"]):
                return list(selected.values())
            if add_track(r["veh_id"]):
                return list(selected.values())

    # 2??: ROI ?/?? ???
    for b, t, c, cf in sorted(roi_persons, key=lambda x: (float(x[0][1]), -x[3]), reverse=True):
        if add_track(t):
            return list(selected.values())

    # 3??: ROI ???? ??? ??
    if ref_persons and vehicles:
        def dist_to_nearest_ref_ped(veh):
            return min(
                (float(veh[0][0] - p[0][0]) ** 2 + float(veh[0][1] - p[0][1]) ** 2)
                for p in ref_persons
            )
        ranked_vehicles = sorted(vehicles, key=dist_to_nearest_ref_ped)
    else:
    ...
    # 4??: ??? ???/??? ?? ???
    remaining_persons = [it for it in persons if it[1] not in selected]
    if vehicles:
        def dist_to_nearest_vehicle(ped):
            return min(
                (float(ped[0][0] - v[0][0]) ** 2 + float(ped[0][1] - v[0][1]) ** 2)
                for v in vehicles
            )
        ranked_remaining = sorted(remaining_persons, key=dist_to_nearest_vehicle)
    else:
        ranked_remaining = sorted(remaining_persons, key=lambda x: x[3], reverse=True)

    for b, t, c, cf in ranked_remaining:
        if add_track(t):
            break
