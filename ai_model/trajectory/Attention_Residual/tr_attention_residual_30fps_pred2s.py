"""
tr_attention_residual_30fps_pred2s.py
=====================================
Attention + Residual LSTM: 1초 관찰 → 2초 예측 @ 30FPS
SEQ=30, PRED=60

구조: Input(17) → Proj(256) → LSTM → Residual+LN → Attention(sqrt(d_k)) → Linear
"""
...
# ==========================================
def load_data():
    print("📂 데이터 로드 중... [30FPS, 1s→2s, Attn+Res]")
...
def save_plots(history):
    ...
    fig.suptitle(f"A-PAS Attn+Res LSTM — 1s → 2s Predict (30FPS)",
    ...
    axes[1,1].set_title("Attention std"); axes[1,1].legend(); axes[1,1].grid(True)

    plt.tight_layout()
    plt.savefig(os.path.join(MODEL_DIR, "training_report_attn_res_30fps_pred2s.png"), dpi=150)
    plt.close()
...
def save_attention_heatmap(model, val_loader, epoch):
    ...
    fig.suptitle(f"Attn+Res (1s→2s) Weights @ Epoch {epoch}")
    for i, ax in enumerate(axes.flat):
        w = attn[i].cpu().numpy()
        ax.bar(range(SEQ_LENGTH), w, color='teal', width=1.0)
        ax.set_ylim(0, max(0.08, w.max()*1.2))
        ax.set_title(f"#{i+1} std={w.std():.4f}")
        ax.axhline(y=1/SEQ_LENGTH, color='red', linestyle='--', alpha=0.4)
    plt.tight_layout()
    plt.savefig(os.path.join(MODEL_DIR, f"attn_res_pred2s_heatmap_ep{epoch:03d}.png"), dpi=100)
    plt.close()
...
def train():
    ...
    for epoch in range(EPOCHS):
        ...
        if av < best_val:
            ...
            torch.save(model.state_dict(),
                       os.path.join(MODEL_DIR, "best_attn_res_30fps_pred2s.pth"))
    ...
    print(f"\n{'='*55}")
    print(f"  Attn+Res LSTM — 1s → 2s Prediction")
    print(f"  Best Val: {best_val:.5f}")
    print(f"  최종 ADE: {history['ade'][-1]:.2f}px | FDE: {history['fde'][-1]:.2f}px")
    print(f"  Attn std: {history['attn_std'][0]:.4f} → {fs:.4f}")
    print(f"  파라미터: {total_params:,}개")
    print(f"{'='*55}")
    if   fs > 0.05: print("  ✅ Attention 활성!")
    elif fs > 0.02: print("  ⚠️ Attention 약함")
    else:           print("  ℹ️ Attention ≈ uniform")
