...
# ==========================================
# ⚙️ [설정] 30FPS Attention + Residual LSTM
# ==========================================
...
MODEL_DIR           = "models/Attention_Residual"
...
# ==========================================
def load_data():
    print("📂 데이터 로드 중... [30FPS Attention+Residual]")
...
# ==========================================
# 🧠 [모델] Attention + Residual LSTM
#
#   변경점 (이전 버전 대비):
#     ✅ FC를 단순 Linear 하나로 롤백 (변인 통제)
#        → 좌표 회귀에 ReLU/Dropout이 방해되는 문제 제거
#     ✅ sqrt(d_k) 스케일링 유지
#     ✅ Residual + LayerNorm 유지
#
#   구조 흐름:
#     Input(17) → Projection(256) → LSTM(256)
#     → Residual(lstm_out + projected) → LayerNorm
#     → Scaled Dot-Product Attention (sqrt(d_k))
#     → concat(h_n[-1], context) → Linear → pred
# ==========================================
class AttentionResidualLSTM(nn.Module):
    def __init__(self):
        super().__init__()

        # Residual: 입력 차원(17) → 히든 차원(256) 프로젝션
        ...
        # ✅ 단순 Linear FC (좌표 회귀에 최적)
        # context(256) + h_n(256) = 512 → 60 (PRED_LENGTH * 2)
        self.fc = nn.Linear(HIDDEN_SIZE * 2, PRED_LENGTH * 2)

    def forward(self, x):
        # 1. Input Projection
        projected = self.input_proj(x)                    # (B, 60, 256)

        # 2. LSTM
        lstm_out, (h_n, _) = self.lstm(projected)         # (B, 60, 256)

        # 3. Residual Connection + LayerNorm
        residual_out = self.layer_norm(lstm_out + projected)  # (B, 60, 256)

        # 4. Scaled Dot-Product Attention
        query  = self.W_q(h_n[-1]).unsqueeze(1)           # (B, 1, 256)
        keys   = self.W_k(residual_out)                    # (B, 60, 256)
        values = self.W_v(residual_out)                    # (B, 60, 256)

        scores = torch.bmm(query, keys.transpose(1, 2)) / self.scale
        attn_weights = F.softmax(scores, dim=-1)           # (B, 1, 60)
        context = torch.bmm(attn_weights, values).squeeze(1)  # (B, 256)

        # 5. Concat → 단순 Linear
        combined = torch.cat([h_n[-1], context], dim=-1)   # (B, 512)
...
def save_plots(history):
    ...
    fig.suptitle("A-PAS Attention+Residual LSTM (sqrt(d_k), Simple FC)",
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(MODEL_DIR, "training_report_attn_res_30fps.png"), dpi=150)
    plt.close()
...
def save_attention_heatmap(model, val_loader, epoch):
    ...
    fig.suptitle(f"Attn+Res Weights @ Epoch {epoch}")
    for i, ax in enumerate(axes.flat):
        w = attn[i].cpu().numpy()
        ax.bar(range(SEQ_LENGTH), w, color='teal', width=1.0)
        ax.set_ylim(0, max(0.05, w.max()*1.2))
        ax.set_title(f"#{i+1} std={w.std():.4f}")
        ax.axhline(y=1/SEQ_LENGTH, color='red', linestyle='--', alpha=0.4)
    plt.tight_layout()
    plt.savefig(os.path.join(MODEL_DIR, f"attn_res_heatmap_ep{epoch:03d}.png"), dpi=100)
    plt.close()
...
def train():
    ...
    model     = AttentionResidualLSTM().to(device)
    ...
    print(f"  📐 구조: Input(17)→Proj(256)→LSTM→Residual+LN→Attention→Linear")
    print(f"  📐 FC: Linear({HIDDEN_SIZE*2} → {PRED_LENGTH*2})  [단순 회귀]")
    ...
    for epoch in range(EPOCHS):
        ...
        if av < best_val:
            ...
            torch.save(model.state_dict(), os.path.join(MODEL_DIR, "best_attn_res_30fps.pth"))
    ...
    else:           print("  ℹ️ Attention ≈ uniform → Residual 단독 전환 권장")
