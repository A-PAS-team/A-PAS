...
# ⚙️ [인자 파싱]
# ==========================================
parser = argparse.ArgumentParser(description="A-PAS Attention+Residual LSTM Training")
...
MODEL_DIR = os.path.join(BASE_DIR, "..", "..", "..", "models", "Attention_Residual")
os.makedirs(MODEL_DIR, exist_ok=True)

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

print("=" * 60)
print(f"  A-PAS Attention + Residual LSTM Training")
print("=" * 60)
print(f"  FPS        : {FPS}")
...
# ==========================================
# 🧠 [Attention + Residual LSTM 모델]
#
# 구조:
#   1. LSTM → 전체 시퀀스 출력
#   2. Dot-product Attention → 중요 타임스텝 집중 → context vector
#   3. FC → offset(Δx, Δy) 예측
#   4. Residual → 마지막 위치 + offset = 최종 예측
# ==========================================
class AttentionResidualLSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        # 3. offset 예측
        offset = self.fc(context).view(-1, PRED_LENGTH, 2)        # (batch, pred, 2)

        # 4. Residual: 마지막 관찰 위치 + offset
        last_pos = x[:, -1, :2].unsqueeze(1)                      # (batch, 1, 2)
        pred     = last_pos + offset                               # (batch, pred, 2)

        return pred, attn_weights, offset
...
# 📈 [그래프 저장]
# ==========================================
def save_plots(history, attn_sample=None, offset_stats=None):
    fig, axes = plt.subplots(1, 4, figsize=(24, 5))
    fig.suptitle(f"A-PAS Attention+Residual LSTM - Training Report ({TAG})", fontsize=14)
...
def train():
    ...
    for epoch in range(EPOCHS):
        ...
        with torch.no_grad():
            for X_batch, y_batch in val_loader:
                X_batch, y_batch    = X_batch.to(device), y_batch.to(device)
                pred, attn, offset  = model(X_batch)
        ...
        if (epoch + 1) % 10 == 0:
            ...
            if last_offset_stats:
                print(f"  🔍 Offset std: Δx={last_offset_stats[2]:.4f}, "
                      f"Δy={last_offset_stats[3]:.4f} "
                      f"({'정상 ✅' if last_offset_stats[2] > 0.001 else '0수렴 ⚠️'})")
    ...
    save_plots(history, last_attn_sample, last_offset_stats)

    print(f"\n{'='*60}")
    print(f"  🎉 학습 완료! [Attention+Residual LSTM - {TAG}]")
    print(f"{'='*60}")
    ...
    if last_attn_sample is not None:
        attn_std = last_attn_sample.std()
        print(f"  Attention std : {attn_std:.4f} {'(집중됨 ✅)' if attn_std > 0.01 else '(균등분포 ⚠️)'}")
    if last_offset_stats:
