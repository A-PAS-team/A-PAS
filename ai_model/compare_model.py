...
MODELS_CONFIG = {
    "Self LSTM"          : ("Self_LSTM",          "self"),
    "Attention"          : ("Attention",           "attention"),
    "Residual"           : ("Residual",            "residual"),
    "Attn+Residual"      : ("Attention_Residual",  "attention_residual"),
}
...
for model_name, (model_dir, model_key) in MODELS_CONFIG.items():
    model_path = os.path.join(BASE_DIR, "..", "models",
                              model_dir, f"best_model_{TAG}.pth")
...
for name, r in results.items():
    ...
    grade      = "✅" if ade_mean < 10 else ("🟡" if ade_mean < 15 else "🔴")
    print(f"{name:<20} {r['params']:>10,} {ade_mean:>9.2f} {fde_mean:>9.2f} "
          f"{ade_median:>11.2f} {ade_10:>8.1f}% {grade}")
print(f"{'='*70}")
...
ax.axhline(10, color='green', linestyle='--', alpha=0.7, label='ADE Target (10px)')
